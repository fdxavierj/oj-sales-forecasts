    ...
    for s in range(10): 
        ...
        for i in range(products_per_store):
            ...
            model = xgb.XGBRegressor(
                    ...
                    max_depth=8,
                    learning_rate=0.1,
                    # Verbosity = 1: warning; means we'll see only warning messages (e.g. improper params)
                    verbosity=1,
                    random_state=0
                )
...
df_results.to_csv("xgboost.csv", index=False)

### end of xgboost regression