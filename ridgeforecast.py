...
### Ridge regression with moving window
...
for t in range(start_second_part, end_second_part):
    for s in range(10):
        ...
        for i in range(products_per_store):
            ...
            time_series_cv = TimeSeriesSplit(n_splits=3, gap=2)
            splits = list(time_series_cv.split(X_training))
            alphas = 10**np.linspace(8, -2, 100) / y_training.std()
...
df_results.to_csv("ridge.csv", index=False)
