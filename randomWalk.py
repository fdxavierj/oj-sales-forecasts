...
for t in range(start, end):
    for s in range(10):
        ...
        for i in range(products_per_store):
            y_random_walk = store_s['sales'][t - 1, i]
            rows_RW.append([s, i, t, y_random_walk])

y_random_walk = pd.DataFrame(rows_RW, columns=['store', 'product', 'time', 'y_randomWalk'])	

y_random_walk = y_random_walk.sort_values(['store', 'product'])
y_random_walk.to_csv('randomWalk.csv', index=False)
